...
%    beta = SPM.xX.pKX*y; % Very close to below (1e-12); line 605 spm_spm.m
%    beta = pinv(SPM.xX.xKXs.X)*y; % Changed by Rik to avoid error if some NaNs
...
% end
% Rik to handle NaNs
[ft,fv] = find(isfinite(y));
fv = unique(fv);

if length(fv) < size(y,2)
    warning('Using modified version of spm_regions.m: excluding %d of %d voxels with at least one NaN',size(y,2)-length(fv), size(y,2))
    y = y(:,fv);
end
[m,n]   = size(y);
if m > n
