...
% for reference
%old=load('/imaging/camcan/cc700/mri/pipeline/release004/data_fMRI_Smooth_SMT_FIR_1s/aamod_firstlevel_model_00005/CC110037/stats/SPM');
...
for sr = 1:length(glms)
    ...
    parfor s = 1:nparticipants
        ...
        SPM.xY.P = spm_select('ExtFPList',fullfile(raw_dir,ccid,'SMT'),'^smswauf.*nii',Inf);
        
        ons = spm_load(fullfile(git_dir,'Onsets',sprintf('sub-%s_epi_smt_onsets.csv.gz',ccid)));
        ...
        cof = spm_load(fullfile(git_dir,'Confounds',sprintf('sub-%s_epi_smt_confounds.csv.gz',ccid)));
...
for sr = 1:length(glms)
    ...
    for s = 1:nparticipants
     ...
     G.user_regs{1} = kron(eye(nBF),detrend(participants.Age,0));
      
     G.contrasts{1}.name = 'HRF';
     G.contrasts{1}.c = [eye(nBF) zeros(nBF)];
     G.contrasts{1}.type = 'F';
     
     G.contrasts{2}.name = 'Age';
     G.contrasts{2}.c = [zeros(nBF) eye(nBF)];
